...
Admin Password: ${_pass}
Generated by: BetterDesk Docker bootstrap${_source}
...
if [ -n "${INIT_ADMIN_PASS:-}" ] || [ -n "${DEFAULT_ADMIN_PASSWORD:-}" ]; then
    ...
    if [ -n "$_env_pass" ]; then
        ...
        if ! file_exists_as_betterdesk "$CREDS_FILE" && ! primary_database_has_users; then
            ...
            _existing_pass=$(parse_creds_password "$CREDS_FILE" || true)
            if [ "$_existing_pass" != "$_env_pass" ]; then
                write_bootstrap_credentials "$_env_pass" " (ADMIN_PASSWORD)"
                echo "Bootstrap admin credentials refreshed → ${CREDS_FILE} (fresh primary database)"
            fi
        fi
    fi
...
if run_as_betterdesk mkdir "$LOCK_DIR" 2>/dev/null; then
    ...
    write_bootstrap_credentials "$_new_pass" ""
