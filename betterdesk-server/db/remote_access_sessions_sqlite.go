...
// CloseOrphanedRemoteAccessSessions closes open remote access sessions that
// cannot still be running, regardless of their source.
//
// Unlike the web-console heartbeat timeout, this makes no assumption about how
// often a session is touched. Sessions recorded from the RustDesk audit feed are
// never touched between their start and their explicit end, so a timeout would
// cut legitimate long sessions short. Instead it relies on two facts that are
// decidable from device presence alone:
//
//   - the target device has no open online session, so nothing can be connected
//     to it, or
